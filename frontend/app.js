    ...
    initCadastroVisitantesPage() {
        ...
        if (form) {
            form.addEventListener('submit', async (e) => {
                ...
                // Gerar data/hora de entrada atual
                const dataHoraEntrada = new Date().toISOString();
                ...
                try {
                    const response = await fetch('http://localhost:8000/api/visitantes', {
                        ...
                        body: JSON.stringify({
                            ...
                            motivo_visita,
                            data_entrada: dataHoraEntrada,
                            observacoes
                        })
                    });
                    const data = await response.json();
                    if (response.ok && data.success) {
                        this.showSuccess('Visitante cadastrado com sucesso!');
                        form.reset();
                        form.classList.remove('was-validated');
                        this.loadVisitantesList();
                    } else {
                        let msg = data.message || 'Erro ao cadastrar visitante.';
                        if (data.errors) {
